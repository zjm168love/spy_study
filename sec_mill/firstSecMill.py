...
driver = webdriver.Chrome(os.path.join(os.path.dirname(__file__), "/Users/pepchou/Downloads/chromedriver"),
...
def buy(buy_time, mall, time_dif):
    ...
    # 转为时间戳
    buyTimeStamp = int(time.mktime(timeArray))
    print("开始准备购买")
...
def get_server_time():
    ...
    x = eval(r1.text)
    tmp = time.time() - localNow
    ali_time = int(x['data']['t'])

    aliTimeStamp = float(ali_time / 1000)
    print(tmp)
    # timeArray = time.localtime(timeStamp)
    # otherStyleTime = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
    return aliTimeStamp, tmp
    # driver.quit()
